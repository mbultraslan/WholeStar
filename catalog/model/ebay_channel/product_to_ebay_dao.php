...
class ModelEbayChannelProductToEbayDao extends Model {
	...
	public function getProductsByEbayItemIds($itemIds) {
		$sQuery = "SELECT pr.*, pd.name, ep.ebay_id FROM "
			. DB_PREFIX . "product pr LEFT JOIN " . DB_PREFIX . "product_description pd ON (pr.product_id = pd.product_id)"
			. " LEFT JOIN `" . DB_PREFIX . "channel_ebay_product` ep on ep.product_id = pr.product_id "
			. " where pd.language_id='" . $this->db->escape($this->config->get('config_language_id')) . "' and pd.product_id = ep.product_id and ep.ebay_id in ('". implode("','", $itemIds) . "') GROUP BY pr.product_id ";
