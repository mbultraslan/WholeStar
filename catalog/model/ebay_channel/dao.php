...
class ModelEbayChannelDao extends Model {
    ...
    public function getEbayAccount() {
        return $this->db->query("SELECT * FROM `" . DB_PREFIX . "channel_ebay_acount` LIMIT 1")->row;
    }
    ...
    public function getListProfileIdByChecksum($md5) {
    ...
    public function getListingProfileMD5($profile, $payments = array(), $shippings = array()) {
    ...
    public function insertListingProfile($profile) {
        $this->db->query(
        ...
        if($profile['is_default']) {
            $this->markAsDefaultListingProfile($id);
        }
        return $id;

    }

    public function updateListingProfile($profile) {
        $this->db->query(
        ...
        if($profile['is_default']) {
            $this->markAsDefaultListingProfile($profile['id']);
        }

        return $profile['id'];
    }

    public function updateListingProfileCheckSum($id, $profile_checksum) {
    ...
    public function updateListingProfileLanguage($id, $language_id) {
    ...
    public function removeListingPaymentMethod($list_profile_id) {
    ...
    public function addListingPaymentMethod($method) {
        $this->db->query(
    ...
    public function removeListingProfile($list_profile_id) {
        $this->removeListingPaymentMethod($list_profile_id);
        $this->removeListingShippingService($list_profile_id);
        $this->db->query("DELETE FROM `" . DB_PREFIX . "channel_ebay_list_profile` where `id`=". $this->db->escape($list_profile_id));
    }

    public function markAsDefaultListingProfile($list_profile_id) {
    ...
    public function removeListingShippingService($list_profile_id) {
    ...
    public function addListingShippingService($service) {
        $this->db->query(
    ...
    public function getListProfileRecord($id) {
        $profile = array();
        ...
        $lss = $this->db->query('SELECT * FROM `' . DB_PREFIX . 'channel_ebay_list_shipping_service` where list_profile_id='. $this->db->escape($id))->rows;
    ...
    public function getEbayFlatDomestic($site_id) {
        return $this->db->query("select shipping_service, description, shipping_carrier from `" . DB_PREFIX . "channel_ebay_shipping_service` where site_id="
            . $this->db->escape($site_id). " and is_flat = 1 and is_international_service = 0 ")->rows;
    }

    public function getEbayCalculatedDomestic($site_id) {
        return $this->db->query("select shipping_service, description, shipping_carrier from `" . DB_PREFIX . "channel_ebay_shipping_service` where site_id="
            . $this->db->escape($site_id). " and is_calculated = 1 and is_international_service = 0 ")->rows;
    }

    public function getEbayFlatInternational($site_id) {
        return $this->db->query("select shipping_service, description, shipping_carrier from `" . DB_PREFIX . "channel_ebay_shipping_service` where site_id="
            . $this->db->escape($site_id). " and is_flat = 1 and is_international_service = 1 ")->rows;
    }

    public function getEbayCalculatedInternational($site_id) {
        return $this->db->query("select shipping_service, description, shipping_carrier from `" . DB_PREFIX . "channel_ebay_shipping_service` where site_id="
            . $this->db->escape($site_id). " and is_calculated = 1 and is_international_service = 1 ")->rows;
    }
    ...
    public function upgradeEbayAccountTable() {
        ...
        $tabel_fields[] = array("Field" => "expirationTime", "Type" => "timestamp", "Null" => "NO", "Default" => "", "Extra" => "");
        $tabel_fields[] = array("Field" => "revocationTime", "Type" => "timestamp", "Null" => "NO", "Default" => "", "Extra" => "");
        ...
        $tabel_fields[] = array("Field" => "dashboard_last_update", "Type" => "timestamp", "Null" => "YES", "Default" => "", "Extra" => "");
    ...
    public function upgradeEbayProductVariationTable() {
        $table_name = 'channel_ebay_variation';
        $tabel_fields = array();
        $tabel_fields[] = array("Field" => "ebay_product_id", "Type" => "int(11)", "Null" => "NO", "Default" => "", "Extra" => "");
        $tabel_fields[] = array("Field" => "qty", "Type" => "int(4)", "Null" => "NO", "Default" => "", "Extra" => "");
        $tabel_fields[] = array("Field" => "sku", "Type" => "varchar(255)", "Null" => "YES", "Default" => "", "Extra" => "");
        $tabel_fields[] = array("Field" => "price", "Type" => "decimal(15,4)", "Null" => "NO", "Default" => "", "Extra" => "");
        $this->upgradeTable($table_name,
            $tabel_fields,
            "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . $table_name."` (`id` int(20) NOT NULL AUTO_INCREMENT, PRIMARY KEY (`id`)) DEFAULT COLLATE=utf8_general_ci;");
    ...
    public function updateOrInstall(){
        ...
        $this->upgradeEbayProductVariationTable();
    ...
    private function installModifications() {
        $xml = file_get_contents(DIR_APPLICATION . 'model/ebay_channel/vq_ebay_channel.xml');
        if ($xml) {
    ...
    public function uninstall(){
        ...
        if (file_exists($path_parts['dirname'] . '/vqmod/xml/vq_ebay_channel.xml')) {
            unlink($path_parts['dirname'] . '/vqmod/xml/vq_ebay_channel.xml');
        }
