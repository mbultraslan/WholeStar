...
class ModelModuleKulerFilter extends Model
{
	...
	public function getProductOptions(array $conditions = array())
	{
		...
		// Query
		$query = $this->db->query("
			SELECT po.product_option_id, po.option_id, po.value, pov.product_option_value_id, pov.option_value_id, COUNT(po.product_id) AS total_product
			...
			$join_tables
			WHERE $where
			GROUP BY po.option_id, po.value, pov.option_value_id
		");
	...
	public function prepareProductConditions(array $conditions)
	{
		...
		if (!empty($conditions['option_value']))
		{
			...
			foreach ($conditions['option_value'] as $option_value)
			{
				...
				$option_conditions[] = "(po.option_id = $option_id AND po.value = $value)";
			}
