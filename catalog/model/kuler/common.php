...

{
	...
	public function getProducts(array $data) {
		...
		if ($data['type'] == self::TYPE_LATEST || $data['type'] == self::TYPE_POPULAR) {
			...
			$order_by = 'p.date_available DESC';
	...
	public function getProductDeal($product_id) {
		...
		if ($query->num_rows) {
			...
			return array(
                ...
                'colours' => $this->model_catalog_product->getProductColours($query->row['product_id']),
...

	{
		...
		$product_data = array(
            ...
            'packQty' => array_sum(explode('-', $product['ratio'])),
