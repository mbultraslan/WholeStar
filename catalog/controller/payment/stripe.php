...
class ControllerPaymentStripe extends Controller {
	...
	public function send() {
		...
		if (!empty($charge_response)) {
			...
			$comment .= $strong . 'Card Type:</strong>' . $charge_response['card']['brand'] . '<br />';
	...
	private function replaceShortcodes($text, $order_info) {
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			foreach ($product['option'] as $option) {
				$options[] = $option['name'] . ': ' . (version_compare(VERSION, '2.0') < 0 ? $option['option_value'] : $option['value']);
			}
	...
	public function webhook() {
		...
		if ($event->type == 'charge.refunded') {
			...
			$country = (isset($customer_response['default_card']['address_country'])) ? $this->db->query("SELECT * FROM " . DB_PREFIX . "country WHERE `name` = '" . $this->db->escape($customer_response['default_card']['address_country']) . "'") : '';
			$country_id = (isset($country->row['country_id'])) ? $country->row['country_id'] : 0;
			
			$zone = (isset($customer_response['default_card']['address_state'])) ? $this->db->query("SELECT * FROM " . DB_PREFIX . "zone WHERE `name` = '" . $this->db->escape($customer_response['default_card']['address_state']) . "' AND country_id = " . (int)$country_id) : '';
