	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('common/home')
				...
				$data['thumb'] = '';
				$data['description'] = $this->language->get('text_product_listing');
			$data['compare'] = $this->url->link('product/compare');
$data['categories'] = array();

		$this->document->setTitle($this->language->get('text_product_listing'));
		$this->document->setDescription($this->language->get('text_product_listing'));
		$this->document->setKeywords($this->language->get('text_product_listing'));

		$data['heading_title'] = $this->language->get('text_product_listing');
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('All Products'),
			'href' => $this->url->link('product/allproduct&limit=100')
		);
		...
		foreach ($results as $result) {
			...
			$this->document->addLink($this->url->link('product/allproduct', $url . '&page=' . $pagination->page), 'canonical');

			if ($pagination->limit && ceil($pagination->total / $pagination->limit) > $pagination->page) {
				$this->document->addLink($this->url->link('product/allproduct', $url . '&page=' . ($pagination->page + 1)), 'next');
			}

			if ($pagination->page > 1) {
				$this->document->addLink($this->url->link('product/allproduct', $url . '&page=' . ($pagination->page - 1)), 'prev');
			}
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/category.tpl')) {
				...
				$this->response->setOutput($this->load->view('default/template/product/allproduct.tpl', $data));
			}
	}
}