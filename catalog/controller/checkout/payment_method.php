...
class ControllerCheckoutPaymentMethod extends Controller {
	public function index() {
		...
		if($this->customer->getEmail() != ''){
			$cemail = $this->customer->getEmail();
		} else {
			$cemail = $this->session->data['guest']['email'];			
		}
		//print_r($this->session->data); die('adasdadas');
		//echo $customer_id = $this->session->data['customer_id'];
		$this->load->model('checkout/order'); 
		$this->load->model('account/newslettersubscribe'); 
		$newsletter = $this->model_account_newslettersubscribe->checkmailidgetdata($cemail);
		if(count($newsletter) > 0){	
		$date = $newsletter[0]['date_added'];		
		$nooforder = $this->model_checkout_order->getOrderBycustomer($cemail,$date); 	
		if($nooforder > 0){
			$this->session->data['coupon'] = '';
		} else {
			$this->session->data['coupon'] = 'FIRST15';
		} }
