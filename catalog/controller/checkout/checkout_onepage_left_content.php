...
//            if (!in_array($acc, array('returning-customer', 'register', 'guest')) || (!$guest_checkout && $acc == 'guest')) {
        ...
        if ($this->customer->isLogged()) {
            if ($this->cart->hasShipping()) {
                $view_data['shippingAddress'] = $this->load->controller('checkout/shipping_address_onepage', true);
            }
            $view_data['paymentAddress'] = $this->load->controller('checkout/payment_address_onepage', true);
        } else {
            $view_data['login'] = $this->load->controller('checkout/login_onepage', true);
            $view_data['register'] = $this->load->controller('checkout/register_onepage', true);
            if ($guest_checkout) {
                $view_data['guest'] = $this->load->controller('checkout/guest_onepage', true);
                if ($this->cart->hasShipping()) {
                    $view_data['guestShipping'] = $this->load->controller('checkout/guest_shipping_onepage', true);
                }
            }
        }
        // </editor-fold>
        if (version_compare(VERSION, '2.2.0.0') < 0) {
            ...
            $template = 'checkout/checkout_onepage_left_content';
        }
