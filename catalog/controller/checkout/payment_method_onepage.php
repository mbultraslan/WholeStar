...
class ControllerCheckoutPaymentMethodOnepage extends Controller {
    ...
    public function index($render_content = false) {
        ...
        $view_data['css'] = $this->model_checkout_checkout_onepage->getcheckoutCss();
        $view_data['themes'] = $view_data['css']['checkout_theme'];
        $view_data['warning_non_address_payment'] = '';


        if ($this->model_checkout_checkout_onepage->calculate_payment()) {
            $this->session->data['payment_methods'] = array();
            $view_data['warning_non_address_payment'] = $this->model_checkout_checkout_onepage->getcheckoutLangugues('warning_non_address_payment');
        } else {
        ...
        if (version_compare(VERSION, '2.2.0.0') < 0) {
            ...
            $template = 'checkout/payment_method_onepage';
        }
