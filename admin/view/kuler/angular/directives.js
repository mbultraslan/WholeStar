...
KulerModule.directive('editor', function ($rootScope) {
	return {
		require: '?ngModel',
		controller: function ($scope, $element, $attrs) {
	...
	return {
		...
		controller: function ($scope) {
			...
			$scope.browse = function () {
				...
				$.ajax({
					url: 'index.php?route=common/filemanager&token=' + getURLVar('token') + '&target=' + fieldId,
					dataType: 'html',
					success: function(html) {
						...
						$('body')
							.off('click.kuler-thumbnail')
							.on('click.kuler-thumbnail', 'a.thumbnail', function () {
							$scope.$apply(function () {
								$scope.image = $('#' + fieldId).val();
							});
						});
			...
			$scope.$watch('image', function (value) {
				if (value) {
					...
					$scope.thumb = 'view/kuler/image/no_image.gif';
				}
			});
		}
	};
...
KulerModule.directive('buttonImageSelector', function () {
	var idCounter = 0;
	...
	return {
		...
		controller: function ($scope) {
			...
			$scope.input_id = 'button_image_selector_' + ++idCounter;
		...
		link: function (scope, element) {
			element.after('<input type="hidden" id="'+ scope.input_id +'" />');
