...
{
	$options['column'] = !empty($options['column']) ? $options['column'] : 8;
...
function renderInput(array $options)
{
	...
	$html .= "<input type=\"{$options['format']}\" class=\"form-control\" name=\"{$options['name']}\" ng-model=\"{$options['name']}\" {$options['inputAttrs']}/>";
...
function renderEditor(array $options)
{
	...
	$html .= "<summernote ng-model=\"{$options['name']}\" height=\"300\"></summernote>";
