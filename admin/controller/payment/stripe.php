...
class ControllerPaymentStripe extends Controller { 
	...
	public function index() {
				...
				foreach ($plan_response['data'] as $plan) {
					...
					foreach ($product_query->rows as $product) {
						$data['subscription_products'][] = array(
							...
							'charge'		=> $this->currency->format($plan['amount'] / pow(10, $this->currency->getDecimalPlace(strtoupper($plan['currency']))), strtoupper($plan['currency']), 1)
						);
					}
				}
			}
		...
		$data['settings'][] = array(
			'key'		=> 'test_secret_key',
			'type'		=> 'text',
			'attributes'=> array('onblur' => '$(this).val($(this).val().trim())', 'style' => 'width: 350px !important'),
		);
		$data['settings'][] = array(
		...
		$data['settings'][] = array(
			'key'		=> 'charge_mode',
			'type'		=> 'select',
			'options'	=> array('authorize' => $data['text_authorize'], 'capture' => $data['text_authorize_and_capture']),
		);
		$data['settings'][] = array(
		...
		$data['settings'][] = array(
			...
			'default'	=> (version_compare(VERSION, '2.0') < 0) ? 'image/no_image.jpg' : 'image/no_image.png',
		);
		$data['settings'][] = array(
			'key'		=> 'checkout_title',
			'type'		=> 'multilingual_text',
		...
		foreach ($data['subscription_products'] as $product) {
			...
			$subscription_products_table .= '
				<tr>
					<td class="left"><a target="_blank" href="index.php?route=catalog/product/' . (version_compare(VERSION, '2.0') < 0 ? 'update' : 'edit') . '&amp;product_id=' . $product['product_id'] . '&amp;token=' . $token . '">' . $product['name'] . '</a></td>
	...
	private function curlRequest($api, $data = array(), $type = 'GET') {
		...
		curl_setopt($curl, CURLOPT_HTTPHEADER, array('Stripe-Version: 2015-02-16'));
	...
	public function refund() {
		$refund_response = $this->curlRequest('charges/' . $this->request->get['charge_id'] . '/refund', array('amount' => $this->request->get['amount']), 'POST');
