...
class ControllerCatalogadvprodfilter extends Controller { 
	...
	public function index() {
		...
		$data['apftxt_results_text_confirm'] = $this->language->get('apftxt_results_text_confirm');
		...
		$data['insert'] = $this->url->link('catalog/product/add', 'token=' . $this->session->data['token'] . '&advprodfilter=1', 'SSL');
		...
		if (isset($post_var['filter_attr'])) {
    		...
    		if (isset($post_var['max_prod_per_pag'])) {
      			...
      			$data['max_prod_per_pag'] = $this->config->get('config_limit_admin');
    		}
...
if (isset($post_var['max_prod_per_pag'])) {
	if ($post_var['max_prod_per_pag']!='' AND (int)$post_var['max_prod_per_pag']>0) {
		...
		$max_prod_per_pag=$this->config->get('config_limit_admin');
	}
} else {
	$max_prod_per_pag=$this->config->get('config_limit_admin');
}
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($product_total) ? (($page - 1) * $max_prod_per_pag) + 1 : 0, ((($page - 1) * $max_prod_per_pag) > ($product_total - $max_prod_per_pag)) ? $product_total : ((($page - 1) * $max_prod_per_pag) + $max_prod_per_pag), $product_total, ceil($product_total / $max_prod_per_pag));
		...
		$datatmp = array(
			...
			if (isset($datatmp['sort']) && in_array($datatmp['sort'], $sort_data)) {
				$final_query .= " ORDER BY " . $datatmp['sort'];	
			} else {
				$final_query .= " ORDER BY pd.name";	
			}
			
			if (isset($datatmp['order']) && ($datatmp['order'] == 'DESC')) {
				$final_query .= " DESC";
			} else {
				$final_query .= " ASC";
			}
		
			if (isset($datatmp['start']) || isset($datatmp['limit'])) {
				if ($datatmp['start'] < 0) {
					$datatmp['start'] = 0;
				}				
				...
				$final_query .= " LIMIT " . (int)$datatmp['start'] . "," . (int)$datatmp['limit'];
			}	
		//////
...
if (count($arr_lista_prod_tmp)>0) {
	...
	for ($i=0;$i<count($arr_lista_prod_tmp);$i++) {
		...
		$arr_lista_prod_tmp[$i]['href_edit'] = $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $arr_lista_prod_tmp[$i]['product_id'] . '&advprodfilter=1', 'SSL');
	}
}
