...
class ControllerModuleKulerMenu extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['status'])) {
			...
			if ($kuler_menu = $this->model_setting_setting->getSetting('kuler_menu', $this->data['selected_store_id'])) {
				if (isset($kuler_menu['kuler_menu_status'])) {
					$this->data['status'] = $kuler_menu['kuler_menu_status'];
				}
			}
		}
		...
		$this->response->setOutput($this->load->view('module/kuler_menu.tpl', $this->data));
	}
	...
	protected function saveAction() {
		...
		$setting = array(
			...
			'kuler_menu_status'     => $this->request->post['status']
		);
		...
		if(isset($this->request->post['op']) && $this->request->post['op'] == 'close') {
			...
			$this->response->redirect($this->url->link('module/kuler_menu', 'token=' . $this->session->data['token'] . '&store_id=' . $this->request->post['store_id'], 'SSL'));
		}
	}
