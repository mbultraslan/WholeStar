...
class ControllerModuleKBMModarchive extends Controller
{
	...
	public function index()
	{
		...
		if (!empty($this->request->get['module_id'])) {
			$this->data['action'] = $this->url->link('module/kbm_mod_archive', 'token=' . $this->session->data['token'] . '&module_id='. $this->request->get['module_id']  , 'SSL');
		...
		if (isset($this->request->post['kbm_mod_archive'])) {
			$this->data['kbm_mod_archive'] = $this->request->post['kbm_mod_archive'];
		} elseif (!empty($module_info)) {
			$this->data['kbm_mod_archive'] = $module_info['kbm_mod_archive'];
		} else {
			$this->data['kbm_mod_archive'] = '';
		}
		...
		$this->response->setOutput($this->load->view('module/kbm_mod_archive.tpl', $this->data));
	}
	...
	private function save()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('kbm_mod_archive', $data);
		...
		{
			$this->response->redirect($this->url->link('module/kbm_mod_archive', 'token=' . $this->session->data['token'] . '&module_id=' . $module_id, 'SSL'));
		}
	}
