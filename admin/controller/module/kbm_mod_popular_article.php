...
class ControllerModuleKBMModPopularArticle extends Controller
{
	...
	public function index()
	{
		...
		if (!empty($this->request->get['module_id'])) {
			$this->data['action'] = $this->url->link('module/kbm_mod_popular_article', 'token=' . $this->session->data['token'] . '&module_id='. $this->request->get['module_id']  , 'SSL');
		...
		if (isset($this->request->post['kbm_mod_popular_article'])) {
			$this->data['kbm_mod_popular_article'] = $this->request->post['kbm_mod_popular_article'];
		} elseif (!empty($module_info)) {
			$this->data['kbm_mod_popular_article'] = $module_info['kbm_mod_popular_article'];
		} else {
			$this->data['kbm_mod_popular_article'] = '';
		}
		...
		$this->response->setOutput($this->load->view('module/kbm_mod_popular_article.tpl', $this->data));
	}
	...
	private function save()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('kbm_mod_popular_article', $data);
		...
		{
			$this->response->redirect($this->url->link('module/kbm_mod_popular_article', 'token=' . $this->session->data['token'] . '&module_id=' . $module_id, 'SSL'));
		}
	}
