...
class ControllerModuleKulerCp extends Controller
{
	...
	{
//		$this->common->installModification('cp');
...
//		$this->common->deleteModificationByName('KulerThemes CP');
	}
	...
	public function index()
	{
		...
		// Actions
		$data['module_url']               = $this->common->createLink('module/kuler_cp');
		$data['support_url']              = 'http://support.kulerthemes.com'; // TODO: Take a look at this
		$data['store_url']                = $this->common->createLink('module/kuler_cp/store');
		$data['skin_url']                 = $this->common->createLink('module/kuler_cp/skin');
		$data['save_url']                 = $this->common->createLink('module/kuler_cp/save');
		$data['save_skin_as_url']         = $this->common->createLink('module/kuler_cp/saveSkinAs');
		$data['remove_skin_url']          = $this->common->createLink('module/kuler_cp/removeSkin');
		$data['export_skins_url']         = $this->common->createLink('module/kuler_cp/exportSkins');
		$data['import_skins_url']         = $this->common->createLink('module/kuler_cp/importSkins');
		$data['style_panel_url']          = $this->common->createLink('module/kuler_cp/stylePanel');
		$data['newsletter_lists_url']     = $this->common->createLink('module/kuler_cp/newsletterLists');
		$data['module_url']               = $this->common->createLink('extension/module');

		$data['selected_store_id']                 = isset($this->request->get['store_id']) ? $this->request->get['store_id'] : 0;

		$data['fonts'] = $this->model->getFonts();
		...
		$this->response->setOutput($this->load->view('module/kuler_cp/index.tpl', $data));
	}
	...
	public function stylePanel()
	{
		...
		$data['group_styles']     = $theme_options['styles'];
		$data['style_template']   = $this->model->loadThemeStyleTemplate($theme_id);

		$this->response->setOutput($this->load->view('module/kuler_cp/style_panel.tpl', $data));
	}
	...
	public function store()
	{
		...
		try
		{
			...
			$result = array(
				...
				'theme_id'              => $theme_id,
				'theme_version'         => $theme_options['version'],
	...
	public function save()
	{
		...
		try
		{
			...
			// TODO: Prevent show Kuler CP from design layout page
	...
	public function autocomplete() {
		...
		foreach ($results as $result)
		{
			$items[$result['product_id']] = array(
				...
				'link' => $this->common->createLink('catalog/product/edit', array('product_id' => $result['product_id']))
			);
		}
