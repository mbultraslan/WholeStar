...
class ModelEbayChannelListingProfileDao extends Model {
    ...
    public function insert($profile) {
        ...
        $this->db->query(
            ...
            . "', `max_qty_to_sell` = '" . $this->db->escape($profile['max_qty_to_sell'])
            ...
            . "', `ean_enabled` = '" . $this->db->escape($profile['ean_enabled'])
            . "', `upc_enabled` = '" . $this->db->escape($profile['upc_enabled'])
            . "', `isbn_enabled` = '" . $this->db->escape($profile['isbn_enabled'])
            . "', `brandmpn_enabled` = '" . $this->db->escape($profile['brandmpn_enabled'])
            . "', `item_specifics` = '" . $this->db->escape($profile['item_specifics'])
    ...
    public function rename($list_profile_id, $name) {
        $this->db->query("UPDATE `" . DB_PREFIX . "channel_ebay_list_profile` SET `name` = '" . $this->db->escape($name) . "' where id = " . (int)$list_profile_id);
    }
    ...
    public function toDTO($profile) {
        ...
        $listProfile['max_qty_to_sell'] = $this->getValue($profile['list_profile'], 'max_qty_to_sell');
        ...
        $listProfile['item_specifics'] = json_decode($profile['list_profile']['item_specifics'], true);
    ...
    public function validateStep2($data, $features, $nameRecommendations) {
        $error = array();
...
//        }
        ...
        //Item specifics
        $item_specifics = (isset($data['item_specific']))? $data['item_specific'] : array();
        $category_specifics = array();
        foreach ($nameRecommendations as $recomandation) {
            $category_specifics[] = array(
                'name' => $recomandation['Name'],
                'req' => ($recomandation['MinValues'] > 0)? true : false,
                'values' => $recomandation['Values'],
            );
        }
        ...
        if(isset($features['EANEnabled']) && !empty($features['EANEnabled'])) {
            $category_specifics[] = array(
                'name' => 'ean_required',
                'req' => ($features['EANEnabled'] == 'Required')? true : false,
                'values' => array(),
            );
        }
        ...
        if(isset($features['UPCEnabled']) && !empty($features['UPCEnabled'])) {
            $category_specifics[] = array(
                'name' => 'upc_required',
                'req' => ($features['UPCEnabled'] == 'Required')? true : false,
                'values' => array(),
            );
        }

        foreach($category_specifics as $category_specific) {
            $req = $category_specific['req'];
            foreach($item_specifics as $name => $value) {
                if($category_specific['name'] == $name && !empty($value) && $value != '00') {
                    $req = false;
                }

                if($category_specific['name'] .'_custom_value' == $name && !empty($value)) {
                    $req = false;
                }
            ...
            if($req) {
                $error['error_item_specific'][$category_specific['name']] = $this->language->get('error_returns_within');
            }
        }
...
//        }
    ...
    public function toEntity($data, $sessionData, $features) {
        ...
        $listProfile['max_qty_to_sell'] = $this->getValue($data, 'max_qty_to_sell', 1);
        ...
        $item_specifics = (isset($data['item_specific']))? $data['item_specific'] : array();
        $category_specifics = array();
        if(isset($sessionData['category_specifics']['NameRecommendations'])) {
            foreach ($sessionData['category_specifics']['NameRecommendations'] as $recomandation) {
                $category_specifics[] = array(
                    'name' => $recomandation['Name'],
                    'req' => ($recomandation['MinValues'] > 0)? true : false,
                    'values' => $recomandation['Values'],
        ...
        $listProfile['item_specifics'] = array();
        foreach($item_specifics as $name => $value) {
            if(!empty($value)) {
                $listProfile['item_specifics'][$name] = $value;
            }
        }
...
//        foreach($category_specifics as $category_specific) {
//            foreach($item_specifics as $name => $value) {
//                if($category_specific['name'] == $name && !empty($value)) {
//                    $listProfile['item_specifics'][$name] = $value;
//                }
//
//                if($category_specific['name'] . '_custom_value' == $name && !empty($value)) {
...
//        }
        ...
        $listProfile['item_specifics'] = json_encode($listProfile['item_specifics']);
    ...
    public function getOptions() {
        return $this->db->query("SELECT * FROM `" . DB_PREFIX . "option_description` where language_id = ". (int) $this->config->get('config_language_id') . " order by name")->rows;
    }

    public function getAttributes() {
        return $this->db->query("SELECT * FROM `" . DB_PREFIX . "attribute_description` where language_id = ". (int) $this->config->get('config_language_id') . " order by name")->rows;
    }

    public function getProductFields() {
        return array(
            array("id" => "model", "name" => "Model"),
            array("id" => "sku", "name" => "SKU"),
            array("id" => "ean", "name" => "EAN"),
            array("id" => "upc", "name" => "UPC"),
            array("id" => "isbn", "name" => "ISBN"),
            array("id" => "mpn", "name" => "MPN"),
            array("id" => "manufacturer_id", "name" => "Manufacturer"),
            array("id" => "location", "name" => "Location")
        );
    }
