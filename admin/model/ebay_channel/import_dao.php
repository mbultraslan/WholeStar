...
class ModelEbayChannelImportDao extends Model {
    ...
    public function importOrder($order, $settings, $processId=null) {
        ...
        if(isset($order['shipping_address']['name']) && !empty($order['shipping_address']['name'])) {
            ...
            $data['firstname'] = $order['shipping_address']['name'];
        ...
        foreach ($order['transactions'] as $transaction) {
            ...
            $product_data[] = array(
                ...
                'total'      => round($transaction['transaction_price'] * $transaction['quantity'], 2),
        ...
        $data['totals'][] = array('code' => 'sub_total', 'title'=> 'Sub-Total', 'text' => $order['subtotal'], 'value'=> $order['subtotal'], 'sort_order'=> 1);

        if(isset($order['shipping_service']['shipping_service_cost'])) {

            $data['totals'][] = array('code' => 'shipping', 'title'=> $data['shipping_method'] . ' - cost', 'text' => $order['shipping_service']['shipping_service_cost'], 'value'=>$order['shipping_service']['shipping_service_cost'], 'sort_order'=> 1);

        }

        if(isset($order['shipping_service']['shipping_service_additional_cost'])) {

            $data['totals'][] = array('code' => 'shipping', 'title'=> $data['shipping_method'] . ' - additional cost', 'text' => $order['shipping_service']['shipping_service_additional_cost'], 'value'=>$order['shipping_service']['shipping_service_additional_cost'], 'sort_order'=> 1);

        }

        $data['totals'][] = array('code' => 'total', 'title'=> 'Total', 'text' => $order['total'], 'value'=>$order['total'], 'sort_order'=> 1);
