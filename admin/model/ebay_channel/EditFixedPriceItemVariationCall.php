...
<?php
class EditFixedPriceItemVariationCall extends BaseCall {
	...
	public function editVariation($itemId, $variations, $profile) {
		...
		foreach ($variations as $variation) {
			...
			$requestXmlBody .= ' <Quantity>0</Quantity>';
			$requestXmlBody .= '<VariationSpecifics>';
			foreach ($variation['variation_specifics'] as $variationSpecific) {
				$requestXmlBody .= '<NameValueList><Name>' . $variationSpecific['name'] . '</Name><Value>'. $variationSpecific['value'] .'</Value></NameValueList>';
			}
			$requestXmlBody .= '</VariationSpecifics>';
			$requestXmlBody .= ' </Variation>';
		}
