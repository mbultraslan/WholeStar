...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['text_option_stock'] = $this->language->get('text_option_stock');
			...
			if ($product_info['image']) {
				...
				$data['popup'] = $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
			...
			if ($product_info['image']) {
				...
				$data['thumb'] = $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'));
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				...
				foreach ($option['product_option_value'] as $option_value) {
					...
					//if (!$option_value['subtract'] || is_numeric($option_value['quantity'])) {
				...
				$modeltype = explode(' ',$product_info['model']);

				$psamecolor = $this->model_catalog_product->getProductByModal($modeltype[0]);

				$data['samecolortype'] = array();

				if(count($psamecolor) > 0){

					foreach($psamecolor as $pcm){

						if($pcm['product_id'] != $data['product_id']){

					     $pcmimg = $this->model_catalog_product->getProductImages($pcm['product_id']);

						 $pcm['images'] = array();

						foreach ($pcmimg as $relimg) {

							$pcm['images'][] = array(

								'popup' => $this->model_tool_image->resize($relimg['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height')),

								'thumb' => $this->model_tool_image->resize($relimg['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'))

							);

						}

							$pcm['href'] = $this->url->link('product/product', 'product_id=' . $pcm['product_id']);

							$data['samecolortype'][] = $pcm;
				...
				//echo '<pre>'; print_r($data['samecolortype']); 
			...
			$data['description'] = str_replace('http://','https://',html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8'));
