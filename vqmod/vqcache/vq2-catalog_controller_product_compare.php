		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
				if ($product_info['image']) {
					$image = $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $this->config->get('config_image_compare_width'), $this->config->get('config_image_compare_height'));
				} else {
					$image = false;
				}
