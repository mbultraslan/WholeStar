	...
	class ControllerModuleKulerCp extends Controller
	{
		...
		public function quick_view()
		{
			...
			if ($product_info) {
				...
				if ($product_info['image']) {
					$this->data['popup'] = $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
				} else {
					$this->data['popup'] = '';
				}

				if ($product_info['image']) {
					$this->data['thumb'] = $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'));
				} else {
					$this->data['thumb'] = '';
				}
				...
				foreach ($results as $result) {
					$this->data['images'][] = array(
						'popup' => $this->model_tool_image->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height')),
						'thumb' => $this->model_tool_image->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'))
					);
				}
				...
				foreach ($results as $result) {
					if ($result['image']) {
						$image = $this->model_tool_image->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height'));
					} else {
				...
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/product/quick_view.tpl', $this->data));
			} else {
				$url = '';
		...
		public function onePageCheckoutValidate()
		{
			try {
				...
				if ($this->cart->hasShipping() && empty($this->request->post['shipping_address_same']) && empty($this->request->post['shipping_address_id'])) {
				...
				if ($this->cart->hasShipping() && empty($this->request->post['shipping_method'])) {
				...
				if (!$this->customer->isLogged() && !empty($this->request->post['create_new_account'])) {
					...
					if ($this->config->get('config_account_id')) {
						...
						if ($information_info && !isset($this->request->post['register_agree'])) {
		...
		{
			// Ignore if the cart does not require shipping
			...
			return $this->load->view($this->config->get('config_template') . '/template/checkout/shipping_method.tpl', $this->data);
		...
		protected function getPaymentMethod()
		{
			...
			return $this->load->view($this->config->get('config_template') . '/template/checkout/payment_method.tpl', $this->data);
		...
		protected function getOrderTotal()
		{
			...
			try
			{
				...
				$order_data = $this->createOrEditOrder();
		...
		{
			if (!empty($this->request->request['country_id'])) {
				if (!empty($this->request->request['shipping_address_same'])) {
					$shipping_address = $this->request->request;
				} else {
					$address_id = $this->request->request['shipping_address_id'];
			...
			$shipping_address = $this->prepareAddress($shipping_address);
		...
		protected function getPaymentAddress()
		{
			if (!empty($this->request->request['country_id']))
			{
				if (!empty($this->request->request['payment_address_same']))
				{
					$payment_address = $this->request->request;
				}
				else
				{
					$address_id = $this->request->request['payment_address_id'];
			...
			$payment_address = $this->prepareAddress($payment_address);

			return $payment_address;
		}

		protected function prepareAddress(array $address) {
			if (empty($address['country']) && !empty($address['country_id'])) {
			...
			if (empty($address['zone']) && !empty($address['zone_id'])) {
		...
		protected function createOrEditOrder() {
			$order_data = array();
			...
			$order_data['firstname'] = empty($this->request->request['firstname']) ? '' : $this->request->request['firstname'];
			$order_data['lastname'] = empty($this->request->request['lastname']) ? '' : $this->request->request['lastname'];
			$order_data['email'] = empty($this->request->request['email']) ? '' : $this->request->request['email'];
			$order_data['telephone'] = empty($this->request->request['telephone']) ? '' : $this->request->request['telephone'];
			$order_data['fax'] = empty($this->request->request['fax']) ? '' : $this->request->request['fax'];
			$order_data['custom_field'] = empty($this->request->request['custom_field']) ? array() : $this->request->request['custom_field'];
			...
			if ($this->customer->isLogged()) {
				...
				$order_data['customer_group_id'] = empty($this->request->request['customer_group_id']) ? $this->config->get('config_customer_group_id') : $this->request->request['customer_group_id'];

				$this->session->data['account'] = 'guest';

				$this->session->data['guest']['customer_group_id'] = $order_data['customer_group_id'] ;
				$this->session->data['guest']['firstname'] = $order_data['firstname'];
				$this->session->data['guest']['lastname'] = $order_data['lastname'];
				$this->session->data['guest']['email'] = $order_data['email'];
				$this->session->data['guest']['telephone'] = $order_data['telephone'];
				$this->session->data['guest']['fax'] = $order_data['fax'];
			}

			$payment_address = $this->getPaymentAddress();

			$order_data['payment_firstname'] = empty($payment_address['firstname']) ? '' : $payment_address['firstname'];
			$order_data['payment_lastname'] = empty($payment_address['lastname']) ? '' : $payment_address['lastname'];
			$order_data['payment_company'] = empty($payment_address['company']) ? '' : $payment_address['company'];
			$order_data['payment_address_1'] = empty($payment_address['address_1']) ? '' : $payment_address['address_1'];
			$order_data['payment_address_2'] = empty($payment_address['address_2']) ? '' : $payment_address['address_2'];
			$order_data['payment_city'] = empty($payment_address['city']) ? '' : $payment_address['city'];
			$order_data['payment_postcode'] = empty($payment_address['postcode']) ? '' : $payment_address['postcode'];
			$order_data['payment_zone'] = empty($payment_address['zone']) ? '' : $payment_address['zone'];
			$order_data['payment_zone_id'] = empty($payment_address['zone_id']) ? '' : $payment_address['zone_id'];
			$order_data['payment_country'] = empty($payment_address['country']) ? '' : $payment_address['country'];
			$order_data['payment_country_id'] = empty($payment_address['country_id']) ? '' : $payment_address['country_id'];
			$order_data['payment_address_format'] = empty($payment_address['address_format']) ? '' : $payment_address['address_format'];
			$order_data['payment_custom_field'] = empty($payment_address['custom_field']) ? array() : $payment_address['custom_field'];
			...
			if ($this->cart->hasShipping()) {
				...
				$order_data['shipping_firstname'] = empty($shipping_address['firstname']) ? '' : $shipping_address['firstname'];
				$order_data['shipping_lastname'] = empty($shipping_address['lastname']) ? '' : $shipping_address['lastname'];
				$order_data['shipping_company'] = empty($shipping_address['company']) ? '' : $shipping_address['company'];
				$order_data['shipping_address_1'] = empty($shipping_address['address_1']) ? '' : $shipping_address['address_1'];
				$order_data['shipping_address_2'] = empty($shipping_address['address_2']) ? '' : $shipping_address['address_2'];
				$order_data['shipping_city'] = empty($shipping_address['city']) ? '' : $shipping_address['city'];
				$order_data['shipping_postcode'] = empty($shipping_address['postcode']) ? '' : $shipping_address['postcode'];
				$order_data['shipping_zone'] = empty($shipping_address['zone']) ? '' : $shipping_address['zone'];
				$order_data['shipping_zone_id'] = empty($shipping_address['zone_id']) ? '' : $shipping_address['zone_id'];
				$order_data['shipping_country'] = empty($shipping_address['country']) ? '' : $shipping_address['country'];
				$order_data['shipping_country_id'] = empty($shipping_address['country_id']) ? '' : $shipping_address['country_id'];
				$order_data['shipping_address_format'] = empty($shipping_address['address_format']) ? '' : $shipping_address['address_format'];
				$order_data['shipping_custom_field'] = empty($shipping_address['custom_field']) ? array() : $shipping_address['custom_field'];
			...
			$order_data['comment'] = empty($this->request->request['comment']) ? '' : $this->request->request['comment'];
			...
			if (!empty($this->session->data['order_id'])) {
				$this->model->deleteOrder($this->session->data['order_id']);
				$order_data['order_id'] = $this->session->data['order_id'];
			}

			$this->session->data['order_id'] = $this->model->addOrder($order_data);
...
SC_SCRIPT;
		...
		public function processStyles()
		{
			...
			// Load Google fonts
			$theme_id = $this->config->get('kuler_cp_theme_id');
			$skin_id = $this->config->get('kuler_cp_skin_id');
		...
		public function onePageCheckoutInit(&$data)
		{
			...
			$coupon_title = $this->language->get('heading_title');
			$entry_coupon = $this->language->get('entry_coupon');

			$this->language->load('checkout/voucher');
			$voucher_title = $this->language->get('heading_title');
			$entry_voucher = $this->language->get('entry_voucher');

			ModelKulerCommon::loadTexts(array(
				'text_use_coupon' => $coupon_title,
				'entry_coupon' => $entry_coupon,
				'text_use_voucher' => $voucher_title,
			...
			if ($this->config->get('config_account_id')) {
				...
				if ($information_info) {
					$data['text_agree_register'] = sprintf($this->language->get('text_agree'), $this->url->link('information/information/agree', 'information_id=' . $this->config->get('config_account_id'), 'SSL'), $information_info['title'], $information_info['title']);
				} else {
