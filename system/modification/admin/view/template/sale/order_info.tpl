...
$('#button-history').on('click', function() {
  ...
  if(typeof verifyStatusChange == 'function'){
	...
	$.ajax({
		url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/order/history&order_id=<?php echo $order_id; ?>&store_id=<?php echo $store_id; ?>',
		type: 'post',
		dataType: 'json',
		data: 'order_status_id=' + encodeURIComponent($('select[name=\'order_status_id\']').val()) + '&notify=' + ($('input[name=\'notify\']').prop('checked') ? 1 : 0) + '&append=' + ($('input[name=\'append\']').prop('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('textarea[name=\'comment\']').val()) + ($('input[name=\'order_summary\']').prop('checked') ? '&order_summary=1' : '') + '&emailtemplate_id=' + $('select[name=\'field_template\']').val(),
		beforeSend: function() {
