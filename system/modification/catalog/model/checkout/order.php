...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "',  each_price = '" . $product['eachPrice'] . "',  pack_price = '" . $product['packPrice'] . "',  ratio = '" . $product['ratio'] . "', ratio_scale = '" . $product['ratioScale'] . "'");
	...
	public function getOrderBycustomer($customer_id,$date) {
		$order_query = $this->db->query("SELECT count(order_id) as nooforder FROM `" . DB_PREFIX . "order` WHERE email = '" . $customer_id . "' AND date_added >= '".$date."'");
		//print_r($order_query); die;
		return $order_query->row['nooforder'];
	}
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach ($order_product_query->rows as $order_product) {
					...
					$size_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$order_product['order_product_id'] . "' AND name = 'Do Not Delete'");
					foreach ($size_option_query->rows as $size_option) {
						$sizeOptions = stripslashes(html_entity_decode($size_option['value']));
						...
						foreach ($sizeOptions['pack'] as $pack) {
							...
							$colour_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_colours WHERE id = '" . (int)$colourId . "'");
							foreach ($colour_option_query->rows as $colour_option) {
								$packQty = $colour_option['quantity'];
								$qty = $pack['qty'];
							}
							$subtracted = array_map(function ($x, $y) {
								return $x - $y;
							}, explode('-', $packQty), explode('-', $packRatio));
							$subtractedPackQty = implode("-", $subtracted);
							$this->db->query("UPDATE " . DB_PREFIX . "product_colours SET pack = (pack - " . (int)$qty . "), quantity = '" . $subtractedPackQty . "' WHERE id = '" . (int)$colourId . "'");
						...
						foreach ($sizeOptions['single'] as $single) {
							...
							$colour_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_colours WHERE id = '" . (int)$colourId . "'");
							foreach ($colour_option_query->rows as $colour_option) {
								$singleQty = $colour_option['single_quantity'];
							}
							$subtracted = array_map(function ($x, $y) {
								return $x - $y;
							}, explode('-', $singleQty), explode('-', $singleRatio));
							$subtractedSingleQty = implode("-", $subtracted);
							$this->db->query("UPDATE " . DB_PREFIX . "product_colours SET  single_quantity = '" . $subtractedSingleQty . "' WHERE id = '" . (int)$colourId . "'");

							$totalSingle += array_sum(explode('-', $singleRatio));
						}

						$totalProduct = $totalPack + $totalSingle;

					}
					$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$totalProduct . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
			...
			if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach($product_query->rows as $product) {
					...
					$size_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$product['order_product_id'] . "' AND name = 'Do Not Delete'");
					foreach ($size_option_query->rows as $size_option) {
						$sizeOptions = stripslashes(html_entity_decode($size_option['value']));
						$sizeOptions = json_decode($sizeOptions, true);
						$totalPack = 0;
					...
					$this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = ( quantity + " . $totalProduct . ") WHERE product_id = '" . (int)$product['product_id'] . "' AND subtract = '1'");
