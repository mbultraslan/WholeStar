...
class Mfilter_Plus {
	...
	public static function getInstance( $ctrl ) {
		if( ! self::$_instance )
			self::$_instance = new Mfilter_Plus( $ctrl );
	...
	public function install() {
		...
		if( $this->_addColumn( 'product', 'mfilter_values', 'TEXT NOT NULL DEFAULT ""' ) ) {
			...
			if( version_compare( $version, '1.0.1', '<' ) ) {
				return true;
			} else if( version_compare( $version, '1.0.3', '<' ) ) {
				$this->db->query("ALTER TABLE `" . DB_PREFIX . "mfilter_values` ADD UNIQUE( `type`, `value`, `value_id` )");
				
				return true;
			} else if( version_compare( $version, '1.0.9', '<' ) ) {
				return true;
			}
	...
	public function installprogress() {
		$steps	= array( 'attribute', 'option', 'product' );
	...
	public function updateProduct( $product_id ) {
		...
		if( $attribs->num_rows || $options->num_rows || ( $filters && $filters->num_rows ) ) {		
			$conditions		= array( 'attribs' => array(), 'options' => array(), 'filters' => array() );
			$items			= array();

			foreach( $attribs->rows as $attrib ) {			
				$attrib['text'] = empty( $this->_settings['attribute_separator'] ) ? array( trim( $attrib['text'] ) ) : array_map( 'trim', explode( $this->_settings['attribute_separator'], $attrib['text'] ) );
		...
		$before = explode( ',', $this->db->query("SELECT * FROM `" . DB_PREFIX . "product` WHERE `product_id` = " . (int) $product_id )->row['mfilter_values'] );
