    ...
    public function getProducts()
    {
        if (!$this->data) {
            foreach ($this->session->data['cart'] as $key => $quantity) {
                ...
                $sizeOptions = array_shift(array_values($product['option']));
                ...
                if ($product_query->num_rows) {
                    ...
                    $eachPrice = $product_query->row['each_price'];
                    $packPrice = $product_query->row['pack_price'];
                    ...
                    // Product Specials
                    $product_special_query = $this->db->query("SELECT price, each_price, pack_price FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1");

                    if ($product_special_query->num_rows) {
                        $price = $product_special_query->row['price'];
                        $eachPrice = $product_special_query->row['each_price'];
                        $packPrice = $product_special_query->row['pack_price'];
                    }
                    ...
                    $this->data[$key] = array(
                        ...
                        'eachPrice' => $eachPrice,
                        'packPrice' => $packPrice,
                        'productRatio' => $product_query->row['ratio'],
                        'productRatioScale' => $product_query->row['ratio_scale'],
                        'packData' => $sizeOptions['pack'],
                        'singleData' => $sizeOptions['single'],
                        'packQty' => $totalPack,
                        'singleQty' => $totalSingle,
                        'price' => ($packPrice * $totalPack + $eachPrice * $totalSingle + $option_price),
                        'total' => ($packPrice * $totalPack + $eachPrice * $totalSingle + $option_price),
